...
![](https://i.imgur.com/3ocVdLX.png)

Historically, we've been using Zabbix to manage our supervising, alerting and on-call duties for supervising a few thousands servers across two datacenters in a mostly static architecture. At this point, we weren't really used to observing our services' response time, error rate or any other specific metrics. With the move to Docker and Mesos/Marathon (an alternative container orchestration platform) and the new mobile website project, we were challenged by the development team in providing meaningful metrics to them so they could perform better deployments, analyze the performance of their new platform and maintain operational excellence. Here's how the SRE team helped them out.
...
![](https://i.imgur.com/di3Ye9K.png)

While working on the project, we quickly found out we were flying in blind with this new platform and technologies and that we needed to be able to observe this new system in order to could reliably and confidently exploit and deliver this project. The dedicated team SRE engineer (we'll write another article explaining how we help other teams deliver better products through DevOps) went out on a mission to overcome this challenge and we chose Prometheus for this. 
...
## Custom instrumenting to the rescue !
...
![rendering timing](https://i.imgur.com/JeAx3A4.png)
...
As word got out that we were experimenting with a new monitoring solution, many teams came to us wanting to expose and show off metrics. Right now, SRE is the only team alongside the mobile website's team using Prometheus (in parallel with the legacy Zabbix system) as we are trying to find the perfect architecture for it to provide the best service to our internal clients. 
